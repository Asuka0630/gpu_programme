...
void batch_infer(std::unordered_map<std::string, T *> &params, T *batch_data, T *blk_out[], T *tmp, T *batch_out, int truncated_length, int batch_size)
{
    ...
    // T *ck_blk = (float *)malloc(batch_size * 128 * sizeof(float));
    // CUDA_CHECK(cudaMemcpy(ck_blk, blk6_out, batch_size * 128 * sizeof(float), cudaMemcpyDeviceToHost));
    // for (int i = 0; i < batch_size; i++)
    // {
    //     for (int j = 0; j < 128; j++)
    //         std::cout << ck_blk[i * 128 + j] << " ";
    //     std::cout << std::endl;
    // }
    // exit(0);
    ...
    // model
    mlp_bn_relu_block_call<float, 32>(blk4_out, params["fc1.weight"], params["fc1.bias"], params["bn1.running_mean"], params["bn1.running_var"], params["bn1.weight"], params["bn1.bias"], blk5_out, batch_size, 1, 1024, 512); // fc1.weight=[512, 1024], fc1.bias=[512], bn1.running_mean=[512], bn1.running_var=[512], bn1.weight=[512], bn1.bias=[512]
    ...
    // T *ck_blk = (float *)malloc(batch_size * 10 * sizeof(float));
    // CUDA_CHECK(cudaMemcpy(ck_blk, batch_out, batch_size * 10 * sizeof(float), cudaMemcpyDeviceToHost));
    // for (int i = 0; i < batch_size; i++)
    // {
    //     for (int j = 0; j < 10; j++)
    //         std::cout << ck_blk[i * 10 + j] << " ";
    //     std::cout << std::endl;
    // }
    // exit(0);
}
...
int main(int argc, char *argv[])
{
    ...
    // freopen("/home/gpu_course/out/cu_out.txt", "w", stdout);
