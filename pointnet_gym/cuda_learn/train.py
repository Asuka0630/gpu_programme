...
class PointCloudDataset(Dataset):
    def __init__(self, root, split, fix_length: int = 0):
        self.list_of_points = []
        self.list_of_labels = []
        self.root = root
        self.split = split
    ...
    def fix_length_statistics_with_median(self):
        ...
        fix_length = (
            int(np.median(lengths)) if self.fix_length == 0 else self.fix_length
        )
        new_list_of_points = []
                ...
                new_list_of_points.append(
                    np.concatenate(
                        (
                            points,
                            np.zeros(
                                (fix_length - points.shape[0], 3), dtype=np.float32
                            ),
                        ),
                        axis=0,
                    )
...
def main():
    ...
    train_dataset = PointCloudDataset(root=data_path, split="train", fix_length=22600)
    test_dataset = PointCloudDataset(root=data_path, split="test", fix_length=22600)
    # 创建 DataLoader 实例
    ...
    test_dataloader = DataLoader(
        test_dataset, batch_size=32, shuffle=False, num_workers=10, drop_last=False
    )
