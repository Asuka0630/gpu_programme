 ...
 */
template <int blockSize = 128, int width = 1>
...
int main(int argc, char *argv[])
{
...
#ifdef DEBUG
    std::string dir = "/home/gpu_course/default_param"; // argv[1];
#endif
#ifndef DEBUG
...
#endif
    int truncated_length = 15360, dataset_len = 1000;
    ...
    /********************************************************************/
    int batch_size = 512, batch_idx = 0;
    half *batch_data;
    int *dataset_lables;
    CUDA_CHECK(cudaMalloc(&batch_data, (size_t)dataset_len * truncated_length * 3 * sizeof(float)));
    ...
    // model
    CUDA_CHECK(cudaMalloc(&infer_out, (size_t)dataset_len * 10 * sizeof(half))); // logsoftmax(10)
    CUDA_CHECK(cudaMalloc(&infer_labels, MAX_LB_POWER2(dataset_len) * 2 * sizeof(int)));
    CUDA_CHECK(cudaMemset(infer_labels + dataset_len, 0, (size_t)(MAX_LB_POWER2(dataset_len) * 2 - dataset_len) * sizeof(int)));
    ...
    for (batch_idx = 0; batch_idx + batch_size <= dataset_len; batch_idx += batch_size)
    {
        batch_infer(params, batch_data + batch_idx * truncated_length * 3, blk_outs, tmp, infer_out + batch_idx * 10, truncated_length, batch_size);
    }
    if (batch_idx < dataset_len)
    {
        batch_idx = dataset_len - batch_size;
        batch_infer(params, batch_data + batch_idx * truncated_length * 3, blk_outs, tmp, infer_out + batch_idx * 10, truncated_length, batch_size);
    }
