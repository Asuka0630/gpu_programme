...
__global__ void Conv1d1k128ci_BN(float *__restrict__ a, float *__restrict__ b, float *__restrict__ c, float *__restrict__ v, float *__restrict__ alpha, float *__restrict__ gamma, const int M, const int N, const int K)
{
    ...
    // 做一次加载
    {
        int load_a_gmem_k = load_a_smem_k;
        int load_a_gmem_addr = OFFSET(load_a_gmem_m, load_a_gmem_k, K);
        FLOAT4(s_a[0][load_a_smem_k][load_a_smem_m]) = FLOAT4(a[load_a_gmem_addr]);
        int load_b_gmem_k = load_b_smem_k;
        int load_b_gmem_addr = OFFSET(load_b_gmem_k, load_b_gmem_n, N);
        FLOAT4(s_b[0][load_b_smem_k][load_b_smem_n]) = FLOAT4(b[load_b_gmem_addr]);
    ...
    for (int bk = 1; bk < (K + BK - 1) / BK; bk++)
    {
        ...
        int load_b_gmem_k = bk * BK + load_b_smem_k;
        int load_b_gmem_addr = OFFSET(load_b_gmem_k, load_b_gmem_n, N);
        FLOAT4(s_a[smem_sel_next][load_a_smem_k][load_a_smem_m]) = FLOAT4(a[load_a_gmem_addr]);
        FLOAT4(s_b[smem_sel_next][load_b_smem_k][load_b_smem_n]) = FLOAT4(b[load_b_gmem_addr]);
        // 计算第i-1个块
#pragma unroll 4
        for (int tk = 0; tk < BK; tk++)
        {
...
#pragma unroll
        for (int tn = 0; tn < TN; tn++)
        {
            int col = (tx * TN / 2) + (tn & 3) + (tn < 4 ? 0 : BN / 2);
            r_c[tm][tn] += s_v[0][col];
    ...
    /*
        ...
        For example:
            ...
            =>
                ...
                grid = {z: B, y: 4, x: C}
    */
    __shared__ T sdata[4][blockSize + 1]; // 4 * (blockSize+4)
    T rdata[4], tmp[4];
    int tid = threadIdx.y;
    int g_idata_planeSize = N * gridDim.x * 4;
    int g_idata_batch_base = g_idata_planeSize * blockIdx.z;
    int line_stride = blockDim.y * gridDim.y;
    int stride = line_stride * gridDim.x * 4;

    int i = g_idata_batch_base + ((blockIdx.y * blockDim.y + threadIdx.y) * gridDim.x + blockIdx.x) * 4;
    FLOAT4(rdata[0]) = FLOAT4(g_idata[i]); // 或 -T_MIN
    int cnt = blockIdx.y * blockDim.y + tid + line_stride;
    i += stride;
    while (cnt < N) // 将不足构成下一个block的部分进行算
    {
        FLOAT4(tmp[0]) = FLOAT4(g_idata[i]);
#pragma unroll
        for (int j = 0; j < 4; j++)
            rdata[j] = fmax(rdata[j], tmp[j]);
        cnt += line_stride;
        i += stride;
    }
#pragma unroll
...
#pragma unroll
            for (int j = 0; j < 4; j++)
                sdata[j][tid] = fmax(sdata[j][tid], sdata[j][tid + 64]);
        }
        __syncthreads();
    }
    if (tid < 32) // last warp unroll
    {
        if (blockSize > 2)
        {
#pragma unroll
            for (int j = 0; j < 4; j++)
                WarpMaxReduce<T, blockSize>(&sdata[j][0], tid);
        }
        else
        {
            sdata[0][0] = fmax(sdata[0][0], sdata[0][1]);
            sdata[1][0] = fmax(sdata[1][0], sdata[1][1]);
            sdata[2][0] = fmax(sdata[2][0], sdata[2][1]);
            sdata[3][0] = fmax(sdata[3][0], sdata[3][1]);
        }
    }
    if (tid == 0)
    {
#pragma unroll
        for (int j = 0; j < 4; j++)
            rdata[j] = sdata[j][tid];
        int g_odata_batch_base = blockIdx.z * gridDim.x * 4 * gridDim.y;
        FLOAT4(g_odata[g_odata_batch_base + (blockIdx.y * gridDim.x + blockIdx.x) * 4]) = FLOAT4(rdata[0]);
    }
}
...
void maxReduce_call(T *input, T *tmp, T *output, int batch_size, int N, int C)
{
    ...
    dim3 gridDim(C / 4, blks, batch_size);
#ifdef DEBUG
    if (blks > 128)
        std::cout << "Too big N or too small blockSize" << std::endl;
#endif
    if (blks > 1)
        _maxReduce<T, blockSize><<<gridDim, blockDim>>>(input, tmp, N);
    else
    {
        // no up level reduce is needed
        _maxReduce<T, blockSize><<<gridDim, blockDim>>>(input, output, N);
        return;
    }
    // TODO: check whether synchronization is required
    // recurse to N = blks
    dim3 blockDim_(1, blks, 1);
    dim3 gridDim_(C / 4, 1, batch_size);
    switch (blockDim_.y)
    {
    // case 1024:
    ...
    //     break;
    case 128:
        _maxReduce<T, 128><<<gridDim_, blockDim_>>>(tmp, output, blks);
        break;
    // case 64:
...
void preProcessBNLayer(std::unordered_map<std::string, float *> params)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        ...
        PreprocessBatchNormParam<float><<<CEIL(bn_layers[i].second, blockSize), blockSize>>>(weight, bias, running_mean, running_var, bn_layers[i].second);
    }
...
void batch_infer(std::unordered_map<std::string, T *> &params, T *batch_data, T *blk_out[], T *tmp, T *batch_out, int truncated_length, int batch_size)
{
    ...
    Linear_call<float, 8>(blk6_out, params["fc3.weight"], params["fc3.bias"], batch_out, batch_size, 1, 256, 10); // fc3.weight=[10, 256], fc3.bias=[10]
